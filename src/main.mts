...
const SUBGRAPH_URLS: Record<string, { decentralized: string }> = {
  ...
  "1": {
    decentralized:
      "https://gateway.thegraph.com/api/[api-key]/subgraphs/id/4TbqVA8p2DoBd5qDbPMwmDZv3CsJjWtxo8nVSqF2tA9a",
  },
};
...
// Define the Market interface to reflect the updated GraphQL response
interface Market {
  outputToken: Token;
...
function transformMarketsToTags(chainId: string, markets: Market[]): ContractTag[] {
  ...
  return validMarkets.map((market) => {
    ...
    return {
      ...
      "Project Name": "Compound V2",
      "UI/Website Link": "https://compound.finance",
      "Public Note": `Compound V2's ${market.outputToken.symbol} (${market.outputToken.name}) token contract.`,
    };
  });
}
